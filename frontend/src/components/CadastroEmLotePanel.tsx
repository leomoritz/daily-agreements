...
export function CadastroEmLotePanel({ onProcessado }: CadastroEmLotePanelProps = {}) {
  ...
  // Painel inicia fechado para manter a tela limpa; o usuário expande
  // para acessar o formulário de cadastro em lote.
  const [expandido, setExpandido] = useState(false);

  const enviando = status === 'enviando';
  const conteudoId = 'cadastro-em-lote-conteudo';
  ...
  return (
    <section className="cadastro-em-lote-panel" aria-labelledby="cadastro-em-lote-titulo">
      <h2 id="cadastro-em-lote-titulo" className="cadastro-em-lote-panel__titulo-wrapper">
        <button
          type="button"
          className="cadastro-em-lote-panel__cabecalho"
          onClick={alternarExpansao}
          aria-expanded={expandido}
          aria-controls={conteudoId}
        >
          <span className="cadastro-em-lote-panel__titulo">Cadastrar Novo Acordo</span>
          <span
            className={
              expandido
                ? 'cadastro-em-lote-panel__seta cadastro-em-lote-panel__seta--aberta'
                : 'cadastro-em-lote-panel__seta'
            }
            aria-hidden="true"
          >
            ▾
      ...
      {expandido && (
        <div id={conteudoId} className="cadastro-em-lote-panel__conteudo">
